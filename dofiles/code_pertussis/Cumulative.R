...
ResultProducer_Cumulative <- function(data, arguments=NULL){
  ...
  if(arguments[["USE_TITLE"]]){
    TITLE <- list(
      "NB"=sprintf("Kumulativt antall meldte pertussis tilfeller fra sesongen %s til %s\n(farget fra %s til %s)",
                   min(seasons),
                   max(seasons),
                   min(colouredSeasons),
                   max(colouredSeasons)),
      "EN"=sprintf("Cumulative number of registered pertussis cases from season %s to %s\n(colored from %s to %s)",
                   min(seasons),
                   max(seasons),
                   min(colouredSeasons),
    ...
    TITLE <- list(
      "NB"=sprintf("Kumulativt antall meldte pertussis tilfeller fra sesongen\n%s til %s (farget fra %s til %s)",
                   min(seasons),
                   max(seasons),
                   min(colouredSeasons),
                   max(colouredSeasons)),
      "EN"=sprintf("Cumulative number of registered pertussis cases from season\n%s to %s (colored from %s to %s)",
                   min(seasons),
                   max(seasons),
                   min(colouredSeasons),
                   max(colouredSeasons)))[[arguments[["LANGUAGE"]]]]
  ...
  TITLE_Y <- list("NB"="Kumulativt antall meldte pertussis tilfeller",
                  "EN"="Cumulative number of registered pertussis cases")[[arguments[["LANGUAGE"]]]]
