...
if [[ "$CI_PWD" != "" ]]; then
  ...
  -e COMPUTER=test \
  raw996/internal_surveillance:test Rscript /git/internal_surveillance/dofiles/test_4_pertussis.R
else
  ...
  internal_rstudio Rscript /git/internal_surveillance/dofiles/test_4_pertussis.R
