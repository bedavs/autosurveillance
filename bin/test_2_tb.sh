...
if [[ "$CI_PWD" != "" ]]; then
  ...
  -e COMPUTER=test \
  raw996/internal_surveillance:test Rscript /git/internal_surveillance/dofiles/test_2_tb.R
else
  ...
  internal_rstudio Rscript /git/internal_surveillance/dofiles/test_2_tb.R
fi
